...
def main(layer,token,segment):  
    ...
    path='data/sample_from_gpt2-large/sample_from_context/token_num8/'
    # layer=1
    # token=1
    x_sample_batch=None
    y_sample_batch=None
    for i in range(60):
        ...
        x_sample=torch.load(path+input_file,map_location=torch.device('cpu')).cuda()
        x_sample=x_sample.permute(1,2,0)
        y_sample=torch.load(path+output_file,map_location=torch.device('cpu')).cuda()
...
if __name__ == '__main__':
    for i_layer in range(1,16):
        # layer=0
        path='.csv'.format(i_layer)
        csvFile=open(path,"w+",newline='')
        name=['segment','token0','token1','token2','token3','token4','token5','token6','token7','token8','token9']
