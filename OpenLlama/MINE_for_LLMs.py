...
class Estimator():
    ...
    def backward(self,batch_size,inputs):
        loss = self.net(batch_size,inputs)
...
def main(layer,token,segment):  
    ...
    #for epoch in range(n_epoch):
        ...
        shuffled_y = y_sample_batch[idx]#!!!this sentence can't be added in forward, otherwise it could lead to cpu time disaster
        ...
        if info_max<info and info==info:
            ...
            print('bad nan searched, jump to the next token')
            break
...
if __name__ == '__main__':
    for i_layer in range(0,26):
        #layer=7
        path='.csv'.format(i_layer)
        csvFile=open(path,"w+",newline='')
        name=['layer','token0','token1','token2','token3','token4','token5']
        writer=csv.writer(csvFile)
        writer.writerow(name)
        csvFile.close()
        for i_segment in range(0,3):
            token_segment=[]
            token_segment.append(i_layer)
