...
from transformers import AutoTokenizer, GPT2LMHeadModel,OpenAIGPTModel,OpenAIGPTTokenizer,GPTNeoXForCausalLM,AutoModelForCausalLM,LlamaForCausalLM,LlamaTokenizer,GPT2Tokenizer,GPT2Model
...
class Sample_with_context_GPT2XL(nn.Module):
    ...
    def forward(self,past_key_values = None,encoder_hidden_states= None,encoder_attention_mask = None,):
        ...
        for s in trange(self.sample_size):
            ...
            hidden_states=self.model.transformer.wte(inputs_token['input_ids'])
    ...
    def random_select(self,batch_size,sample_num):
        with open('data/MIdataset/tokenlist_tensor/{}_batch5000.pth'.format(sample_num),'r') as file:
...
class Sample_with_context_Openllama(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
...
class Sample_with_context_Gemma(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
...
class Sample_with_sequence_GPT2XL(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
    ...
    def forward(self,past_key_values = None,encoder_hidden_states= None,encoder_attention_mask = None,):
        ...
        for s in trange(self.sample_size):
            ...
            #inputs = self.tokenizer("Hello, my dog is cute", return_tensors="pt")
            batch_input_token=torch.load('data/MIdataset/tokenlist_tensor/{}_batch5000.pth'.format(str(s))).to(self.device)
            #batch_input_token=batch_input_token[:,16-self.token_num:]
            hidden_states=self.model.transformer.wte(batch_input_token)
            for i, (block, layer_past) in enumerate(zip(self.layers, past_key_values)):
...
class Sample_with_sequence_GPT2large(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
    ...
    def forward(self,past_key_values = None,encoder_hidden_states= None,encoder_attention_mask = None,position_ids= None):
        ...
        for s in trange(self.sample_size):
            batch_data=self.random_select(self.batch_size,s)
            inputs_token = self.tokenizer(batch_data, return_tensors="pt").to(self.device)
            batch_input_token=torch.load('data/MIdataset/context_tensor_gpt/{}_batch5000_OpenHermes.pth'.format(str(s))).to(self.device)
            batch_input_token=batch_input_token[:,16-self.token_num:]
            
            input_shape = batch_input_token.size()
            if past_key_values is None:
                past_length = 0
                past_key_values = tuple([None] * len(self.layers))
            ...
            inputs_embeds=self.model.wte(batch_input_token)
            position_embeds = self.model.wpe(position_ids)
            hidden_states = inputs_embeds + position_embeds
            for i, (block, layer_past) in enumerate(zip(self.layers, past_key_values)):
...
class Sample_with_sequence_Gemma(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
    ...
    def forward(self,past_key_values = None,cache_position= None,position_ids = None,):
        ...
        for s in trange(self.sample_size):
            ...
            i=0    
            batch_input_token=torch.load('data/MIdataset/context_tensor/{}_batch500_OpenOrca.pth'.format(str(s))).to(self.device)
            batch_input_token=batch_input_token[:,16-self.token_num:]
...
class Sample_with_sequence_Openllama(nn.Module):
    def __init__(self,args):
        super().__init__()
        self.args=args
        self.model_name=args.model_name
