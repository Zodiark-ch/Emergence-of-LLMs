...
parser.add_argument('--model', default='gpt2', type= str, help='gpt2,openllama,gemma')
...
for i in range(len(sample_sequeece)):
    ...
    if input_ids.size()[-1]>20:
        ...
        idx=torch.nonzero(input_ids==13)
        if idx.size()[0]>1:
            sample_flag=1
            for i_idx in range(idx.size()[0]):
                ...
                if squeeze_length>=17:
                    ...
                    if flag==1:
                        ...
                        elif batch_squeeze.size()[0]<5000: 
                            batch_squeeze=torch.cat((batch_squeeze,squeeze),dim=0)
                            # print(i,data_id)
                        elif batch_squeeze.size()[0]==5000 and num<60:
                            print('save {} batch5000 tensor'.format(num),batch_squeeze.size()) 
                            torch.save(batch_squeeze,'data/MIdataset/context_tensor_gpt/{}_batch5000_OpenOrca.pth'.format(num))
